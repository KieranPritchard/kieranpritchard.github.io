...
function blogFilter() {
    ...
    // Get states of the 3 checkboxes
    const boxOneIsChecked = document.getElementById("blog-topic-1").checked;
    const boxTwoIsChecked = document.getElementById("blog-topic-2").checked;
    const boxThreeIsChecked = document.getElementById("blog-topic-3").checked;

    // Build array of active filters
    let activeFilters = [];
    if (boxOneIsChecked) activeFilters.push("topicOne");
    if (boxTwoIsChecked) activeFilters.push("topicTwo");
    if (boxThreeIsChecked) activeFilters.push("topicThree");
    ...
    // Show only posts that match ANY selected filter
    blogPosts.forEach(post => {
        const matches = activeFilters.some(filter => post.classList.contains(filter));
